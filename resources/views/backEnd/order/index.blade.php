                            ...
                            <td>
                              <div class="btn-group" role="group">
                                  <a href="{{ route('admin.order.invoice',['invoice_id'=>$value->invoice_id]) }}" 
                                    class="btn btn-sm btn-info rounded-pill" title="Invoice">
                                      <i class="fe-eye"></i>
                                  </a>

                                  <a href="{{ route('admin.order.process',['invoice_id'=>$value->invoice_id]) }}" 
                                    class="btn btn-sm btn-warning rounded-pill" title="Process">
                                      <i class="fe-settings"></i>
                                  </a>

                                  <a href="{{ route('admin.order.edit',['invoice_id'=>$value->invoice_id]) }}" 
                                    class="btn btn-sm btn-primary rounded-pill" title="Edit">
                                      <i class="fe-edit"></i>
                                  </a>
                                  ...
                                  <form method="post" action="{{ route('admin.order.destroy') }}" class="d-inline">
                                      ...
                                      <button type="submit" title="Delete" 
                                              class="btn btn-sm btn-danger rounded-pill delete-confirm">
...
<script>
  $(document).ready(function(){
    ...
    // 1️⃣ Selected order IDs (checkbox থেকে collect করবে)
    let selectedOrders = [];
    ...
    $(".order_checkbox").on("change", function(){
        ...
        $("#selected_order_ids").val(selectedOrders); // Hidden input
    });

    // 2️⃣ City -> Zone dependent dropdown
    $("#pathaocity").on("change", function(){
        let cityId = $(this).val();
        if(cityId){
            $.get("/pathao/cities/" + cityId, function(res){
                $("#pathaozone").empty().append('<option value="">Select Zone...</option>');
                if(res.data && res.data.data){
                    $.each(res.data.data, function(i, zone){
                        $("#pathaozone").append('<option value="'+zone.zone_id+'">'+zone.zone_name+'</option>');
                    });
                }
                $("#pathaoarea").empty().append('<option value="">Select Area...</option>'); // Reset area
            });
        } else {
            $("#pathaozone").empty().append('<option value="">Select Zone...</option>');
            $("#pathaoarea").empty().append('<option value="">Select Area...</option>');
        }
    });

    // 3️⃣ Zone -> Area dependent dropdown
    $("#pathaozone").on("change", function(){
        let zoneId = $(this).val();
        if(zoneId){
            $.get("/pathao/zones/" + zoneId, function(res){
                $("#pathaoarea").empty().append('<option value="">Select Area...</option>');
                if(res.data && res.data.data){
                    $.each(res.data.data, function(i, area){
    ...
    // 4️⃣ Form submit via AJAX
    $("#order_sendto_pathao").on("submit", function(e){
        e.preventDefault();
                ...
                if(Array.isArray(res)){
                    ...
                    res.forEach(function(item){
                        if(item.status === "success"){
                            successMsg += "Order #"+item.order_id+" ✅ Tracking ID: "+item.tracking_id+"<br>";
                        } else {
                            successMsg += "Order #"+item.order_id+" ❌ Failed: "+item.message+"<br>";
                        }
                    });
                    Swal.fire({
                        icon: "info",
                        title: "Pathao Response",
                        html: successMsg,
                    });
                } else {
