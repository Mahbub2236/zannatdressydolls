        ...
        if ($courier_info) {
            ...
            foreach ($orders_id as $order_id) {
                ...
                if ($request->status == 5 && $courier != 5) {
                    $consignmentData = [
                        'invoice' => $order->invoice_id,
                        'recipient_name' => $order->shipping ? $order->shipping->name : 'Bohisojja',
                        'recipient_phone' => $order->shipping ? $order->shipping->phone : '01838046508',
                        'recipient_address' => $order->shipping ? $order->shipping->address : 'Gorom Panir Goli, Fakirapool, Dhaka-1000',
                        'cod_amount' => $order->amount
                    ];
                    $client = new Client();
                    $response = $client->post('$courier_info->url', [
